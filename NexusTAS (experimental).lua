...
local function toggleRecording()
    ...
    -- === FIX: измеряем реальное время settle-ожиданий ===
    local settleStart = tick()
    RunService.Heartbeat:Wait()
    RunService.RenderStepped:Wait()
    if HUMAN then HUMAN:SetStateEnabled(Enum.HumanoidStateType.Jumping, true) end
    -- Сдвигаем таймлайн на потраченное время,
    -- иначе персонаж успевал пройти 2 физкадра, а clock сдвигался лишь на 1.
    recordingClock = recordingClock + (tick() - settleStart)
...
-- === SINGLE PHYSICS TICK (через штатный unpause/pause) ===
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    elseif inputMatchesBind(input, keybinds.tick) then
        if state == "create" then requestSingleStep() end
